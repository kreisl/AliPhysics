...
void AliZDCflowCuts::AddQAHistograms(TList *list) {
  ...
  fHistTPCchi2perCls = new TH1D("TPCchi2perCls",";tpc chi2 per cls;N", 1000, 0, 10.);
...
bool AliZDCflowCuts::CheckTrackCutsNoPtCut(AliAODTrack* track) {
  if (track->TestFilterBit(filterBit)) {
    ...
    // check cuts
    auto pass_cuts = true;
    if (sign == -1)     pass_cuts = pass_cuts && (track->GetSign() < 0);
    else if( sign == 1) pass_cuts = pass_cuts && (track->GetSign() > 0);
    pass_cuts = pass_cuts && (std::fabs(eta) < etaMax);
    pass_cuts = pass_cuts && (tpc_ncls > nClsCut);
    pass_cuts = pass_cuts && (tpc_chi2_cls > chi2MinCut &&
                              tpc_chi2_cls < chi2MaxCut);
    // Fill QA Histograms after cuts are applied
    if (!pass_cuts) return false;
    if (fQAhistograms) {
